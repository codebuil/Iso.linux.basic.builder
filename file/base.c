...
extern void clears(){
	int n=0;
	char *src = (char *)0x000b8000L;
	for(n=0;n<80*24*2;n=n+2){
		src[n]=32;
		src[n+1]=0x67;
	}
}
extern void scrollb8000()
{
	int n=0;
	int nn=0;
...
extern void copyb8000(int address,char *s)
{
	int n=0;
	int nn=0;
...
void memfill(void *dest, long length, unsigned char value) {
...
void memcopy(void *dest, const void *src, long length) {
...
void memReplace(char* mem, char oldChar, char newChar, long length) {
    for (long i = 0; i < length; i++) {
        if (mem[i] == oldChar) {
            mem[i] = newChar;
        }
